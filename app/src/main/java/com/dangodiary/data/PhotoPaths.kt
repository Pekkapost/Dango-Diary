...
/**
 * Photo paths on an [Entry] are stored as a JSON-encoded string of absolute file paths.
 *
 * Using a single column (vs. a separate photos table) keeps the schema flat — we never query
 * across photos and an entry rarely has more than a handful. If that changes, split the
